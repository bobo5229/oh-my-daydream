...
| 需求 | 打开 |
| --- | --- |
| 写什么 / 不能写什么（安全、可写范围、协作） | `boundaries.md` |
| 怎么写（现代小说、生活气息、禁止的段落风） | `style.md` |
| 用哪个词（硬禁、分档、自检） | `wording.md` |
| 流程：提示词 → 节拍审核 → 正文 | `sop-1.md` |
| 流程：节拍 → 英文草稿 → 翻译入正文 | `sop-2.md`（仅 SOP 2：A = 本助手写英文；B = Gemini 译入正文。SOP 1 不启用 B） |
...
**不维护** `taboos.md` / 雷区。路数与是否展开某玩法由节拍决定；临时加禁写在当次 `beats.md`（只做加法，不能解除 `wording.md` 硬禁）。
...
根目录只放 meta 规则（本文、`boundaries` / `style` / `wording` / `sop-*` 等），不按「故事」在根目录建库。
...
- 未同意节拍前不写正文（见 `sop-1.md`）

### 安全与文风（摘要，细则见专文）

- 性相关角色 **18+**；禁止未成年性化；见 `boundaries.md`
- 中文现代小说 + 生活气息；偏文言词可接受，禁止成句成段文言及其他禁止段落风；见 `style.md`
- 落词前过 `wording.md` 硬禁与分档

### 助手行为

- 保持仓库轻量：不主动搭复杂架构、不写未要求的说明书
- 改规则只改对应文件；路由变更时同步更新本文
